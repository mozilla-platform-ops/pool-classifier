...
class PoolClassifier:
    ...
    def _write_html(
        ...
        parts = [
            ...
            "  .source-controls button { border:0; background:transparent; color:#777; cursor:pointer; font:inherit; padding:0 .25rem; } .source-controls button.active { color:#f90; } .source-chart { display:flex; align-items:end; gap:.35rem; height:12rem; max-width:80rem; padding:.5rem; border:1px solid #333; background:#161616; } .source-day { flex:1; height:100%; display:flex; flex-direction:column-reverse; min-width:1rem; } .source-segment { min-height:1px; cursor:default; outline:1px solid transparent; outline-offset:-1px; transition:filter .12s, outline-color .12s, box-shadow .12s; } .source-segment:hover, .source-segment:focus-visible { position:relative; z-index:1; outline:2px solid #fff; filter:brightness(1.25); box-shadow:inset 0 0 0 1px #161616, 0 0 4px #fff; } .source-legend { display:flex; flex-wrap:wrap; gap:.8rem; color:#aaa; font-size:.8em; margin:.5rem 0; } .source-tooltip { display:none; position:fixed; z-index:10; max-width:22rem; padding:.55rem .7rem; border:1px solid #888; border-radius:3px; background:#111; color:#eee; box-shadow:0 3px 12px #0009; font-size:.82em; line-height:1.35; pointer-events:none; } .source-tooltip.visible { display:block; } .source-tooltip-heading { margin:0 0 .3rem; color:#ccc; } .source-tooltip-current { margin:.2rem 0 .35rem; } .source-tooltip-rows { display:grid; gap:.15rem; } .source-tooltip-row { display:flex; justify-content:space-between; gap:1rem; } .source-tooltip-row.active { font-weight:bold; color:#fff; }",
        ...
        parts += [
            ...
            '<p id="source-freshness" class="gen">Loading job sources…</p><div id="source-legend" class="source-legend"></div><div id="source-chart" class="source-chart" role="group" aria-label="Daily job volume by source"></div><div id="source-tooltip" class="source-tooltip" role="tooltip" aria-hidden="true"></div>',
        ...
        parts += [
            ...
            "  function coverageBreakDetail(events, diagnosticsAvailable) { if (!diagnosticsAvailable) return 'Coverage-break diagnostics could not be loaded.'; if (!events.length) return 'No retained coverage-break event explains this gap.'; return events.map(event => { const workers = event.worker_id || 'unknown worker'; const previous = event.previous_window_count ?? '—', current = event.current_window_count ?? '—', overlap = event.overlap_count ?? '—'; return `${coverageReason(event.reason)}; worker: ${workers}; windows: ${previous} → ${current}; overlap: ${overlap}`; }).join('\\n'); }",
