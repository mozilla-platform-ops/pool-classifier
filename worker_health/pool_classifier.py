...
class PoolClassifier:
    ...
    def _write_html(
        ...
        parts += [
            ...
            "  const sourceChart=document.getElementById('source-chart'), sourceLegend=document.getElementById('source-legend'), sourceFreshness=document.getElementById('source-freshness'), sourceButtons=[...document.querySelectorAll('[data-source-days]')]; let sourceDays=7; const SOURCE_COLORS=['#56b4e9','#e69f00','#009e73','#f0e442','#0072b2','#d55e00','#cc79a7','#88ccee','#aa4499','#44aa99','#ee7733','#332288']; const sourceColor=(source,index)=>source==='unknown'?'#777':SOURCE_COLORS[index%SOURCE_COLORS.length]; function loadSources(){fetch(`${JOB_SOURCES_URL}?days=${sourceDays}`).then(r=>r.ok?r.json():Promise.reject(new Error(`HTTP ${r.status}`))).then(data=>{const sources=[...new Set(data.buckets.map(b=>b.source))].sort(), sourceColors=new Map(sources.map((s,index)=>[s,sourceColor(s,index)])), days=[...new Set(data.buckets.map(b=>b.day))], byDay=new Map(days.map(d=>[d,[]])); data.buckets.forEach(b=>byDay.get(b.day).push(b)); const max=Math.max(...days.map(d=>byDay.get(d).reduce((n,b)=>n+b.tasks,0)),1); sourceChart.innerHTML=days.map(d=>{const rows=byDay.get(d), total=rows.reduce((n,b)=>n+b.tasks,0); return `<div class='source-day' title='${d}: ${total} tasks'>${rows.map(b=>`<div class='source-segment' tabindex='0' style='height:${100*b.tasks/max}%;background:${sourceColors.get(b.source)}' title='${d}\\n${b.source}: ${b.tasks}' aria-label='${esc(`${d}, ${b.source}: ${b.tasks} tasks`)}'></div>`).join('')}</div>`}).join(''); sourceLegend.innerHTML=sources.map(s=>`<span><i class='hm-swatch' style='background:${sourceColors.get(s)}'></i>${esc(s)}</span>`).join(''); sourceFreshness.textContent=`Last ${sourceDays} days · ${data.buckets.reduce((n,b)=>n+b.tasks,0)} terminal runs`;}).catch(e=>{sourceFreshness.textContent=`Job sources unavailable: ${e.message}`;});} sourceButtons.forEach(b=>b.addEventListener('click',()=>{sourceDays=Number(b.dataset.sourceDays);sourceButtons.forEach(x=>x.classList.toggle('active',x===b));loadSources();})); loadSources();",
