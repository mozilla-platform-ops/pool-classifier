...
class PoolClassifier:
    ...
    def _write_html(
        ...
        parts = [
            ...
            "  .lag-chart-wrap { max-width:80rem; overflow-x:auto; border:1px solid #333; background:#161616; padding:.5rem; } .lag-chart { min-width:42rem; height:12rem; display:block; } .lag-compact { color:#888; font-size:.85em; margin:.5rem 0; }",
            ...
            "  .lag-hm-label { color:#777; font-size:.72em; text-align:center; } .lag-hm-day { text-align:left; padding-right:.2rem; align-self:center; } .lag-hm-cell { height:1rem; border:0; border-radius:2px; cursor:default; outline:1px solid #111; } .lag-hm-cell.insufficient { background:repeating-linear-gradient(135deg,#333 0,#333 4px,#222 4px,#222 8px) !important; } .lag-hm-cell:hover { filter:brightness(1.25); outline-color:#aaa; }",
            "</style>",
            "</head>",
            "<body>",
        ...
        parts += [
            ...
            "  function renderLag(data) { const usable = data.buckets.filter(b => b.sufficient_samples); if (!usable.length) { lagChart.innerHTML = `<text x='24' y='35' fill='#888'>Not enough observed terminal runs yet (need ${data.min_samples} per hour).</text>`; return; } const first=data.buckets.indexOf(usable[0]), last=data.buckets.indexOf(usable.at(-1)), span=data.buckets.slice(first,last+1), rawMax=Math.max(data.slo_seconds,...usable.map(b=>b.p95_seconds)), axisStep=rawMax<3600?60:rawMax<=6*3600?1800:rawMax<=24*3600?3600:4*3600, axisMax=Math.ceil(rawMax/axisStep)*axisStep, maxCount=Math.max(...span.map(b=>b.sample_count),1), x=i=>span.length===1?502:70+i*864/(span.length-1), y=v=>185-v/axisMax*145, axisLabel=v=>axisMax>=3600?`${v/3600}h`:fmtLag(v), points=key=>usable.map(b=>`${x(span.indexOf(b)).toFixed(1)},${y(b[key]).toFixed(1)}`).join(' '); const bars=span.map((b,i)=>`<rect x='${x(i)-2}' y='${214-b.sample_count/maxCount*22}' width='4' height='${b.sample_count/maxCount*22}' fill='#555'><title>${b.start_at}\\n${b.sample_count} samples\\np50 ${fmtLag(b.p50_seconds)}, p95 ${fmtLag(b.p95_seconds)}</title></rect>`).join(''); const dots=usable.flatMap(b=>[['p50_seconds','#5dd'],['p95_seconds','#f90']].map(([key,color])=>`<circle cx='${x(span.indexOf(b))}' cy='${y(b[key])}' r='3.5' fill='${color}'><title>${b.start_at}\\n${key === 'p50_seconds' ? 'p50' : 'p95'} ${fmtLag(b[key])}\\n${b.sample_count} samples</title></circle>`)).join(''); const lines=usable.length>=4?`<polyline points='${points('p50_seconds')}' fill='none' stroke='#5dd' stroke-width='2'/><polyline points='${points('p95_seconds')}' fill='none' stroke='#f90' stroke-width='2'/>`:''; const ticks=Array.from({length:axisMax/axisStep+1},(_,i)=>i*axisStep).map(v=>`<line x1='70' y1='${y(v)}' x2='934' y2='${y(v)}' stroke='#2d2d2d'/><line x1='66' y1='${y(v)}' x2='70' y2='${y(v)}' stroke='#666'/>${(axisMax<3600||v%3600===0||v===axisMax)?`<text x='62' y='${y(v)+4}' text-anchor='end' fill='#888' font-size='11'>${axisLabel(v)}</text>`:''}`).join(''); lagChart.innerHTML=`${ticks}<line x1='70' y1='${y(data.slo_seconds)}' x2='934' y2='${y(data.slo_seconds)}' stroke='#f44' stroke-dasharray='5 4'/><text x='74' y='${y(data.slo_seconds)-4}' fill='#f77' font-size='11'>SLO ${fmtLag(data.slo_seconds)}</text><text x='70' y='235' fill='#777' font-size='11'>${span[0].start_at.slice(5,16)} UTC</text><text x='800' y='235' fill='#777' font-size='11'>${span.at(-1).start_at.slice(5,16)} UTC</text>${bars}${lines}${dots}`; }",
            ...
            "    arTimer = enabled ? setTimeout(() => location.reload(), 60000) : null;",
            "  }",
