...
class PoolClassifier:
    ...
    def _write_html(
        ...
        parts = [
            ...
            "  .util-hour-incomplete { background:repeating-linear-gradient(135deg,#303030 0,#303030 4px,#202020 4px,#202020 8px); } .util-hour-partial { outline:2px dashed #d48718; outline-offset:-2px; } .util-hour-collection-interruption { outline:3px double #d946ef; outline-offset:-3px; } .util-hour-unavailable { background:#6b1d1d; } .util-hour-error { background:#555; }",
        ...
        parts += [
            ...
            "  const lagFreshness = document.getElementById('lag-freshness'), lagChart = document.getElementById('lag-chart'), lagChartWrap = document.getElementById('lag-chart-wrap'), lagHeatmap = document.getElementById('lag-heatmap'), lagHeatmapWrap = document.getElementById('lag-heatmap-wrap'); let startLagData = null;",
            ...
            "  const formatLagTime = iso => formatTime(iso, timezoneMode), formatLagAxisTime = iso => timezoneMode === 'utc' ? `${iso.slice(5,16).replace('T',' ')} UTC` : new Date(iso).toLocaleString(undefined, {month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'});",
            "  function formatLagRenderedTimes(data) { const usable=data.buckets.filter(b=>b.sufficient_samples); if(!usable.length)return;const first=data.buckets.indexOf(usable[0]),last=data.buckets.indexOf(usable.at(-1)),span=data.buckets.slice(first,last+1), labels=[...lagChart.querySelectorAll('text')].filter(label=>label.textContent.endsWith(' UTC')); if(labels.length>=2){labels[0].textContent=formatLagAxisTime(span[0].start_at);labels.at(-1).textContent=formatLagAxisTime(span.at(-1).start_at);} lagChart.querySelectorAll('rect.lag-point title').forEach((title,index)=>{const b=span[index];title.textContent=`${formatLagTime(b.start_at)}\\n${b.sample_count} samples\\np50 ${fmtLag(b.p50_seconds)}, p95 ${fmtLag(b.p95_seconds)}`;});lagChart.querySelectorAll('circle.lag-point title').forEach((title,index)=>{const b=usable[Math.floor(index/2)], percentile=index%2?'p95':'p50';title.textContent=`${formatLagTime(b.start_at)}\\n${percentile} ${fmtLag(index%2?b.p95_seconds:b.p50_seconds)}\\n${b.sample_count} samples`;}); }",
            "  const renderLagWithoutTimezone = renderLag; renderLag = data => { renderLagWithoutTimezone(data); formatLagRenderedTimes(data); };",
            ...
            "    if (u.status === 'partial') { const quality = u.utilization_pct > 100 ? `<p class='warn'>Over 100% — possible data-quality issue</p>` : ''; const capacity = u.available_worker_hours > 0 ? `Available: ${u.available_worker_hours.toFixed(2)} worker-hours` : 'No available capacity observed'; const utilization = u.utilization_pct === null ? '—' : `<strong>${u.utilization_pct.toFixed(1)}%</strong> utilization`; const coverage = Number(u.coverage_pct.toFixed(1)); return `<article class='util-card partial'><h3>${label}</h3><p>${utilization}</p><p class='util-detail'>Busy: ${u.busy_worker_hours.toFixed(2)} worker-hours<br>${capacity}<br>Coverage: ${coverage}% (metrics use observed periods only)</p>${quality}</article>`; }",
            ...
            "  }",
            "  const TIMEZONE_STORAGE_KEY = 'pool-classifier:detail:timezone';",
            "  timezoneButtons.forEach(button => button.addEventListener('click', () => { const mode=button.dataset.timezone; setTimezone(mode); try { localStorage.setItem(TIMEZONE_STORAGE_KEY,mode); } catch (_) {} }));",
            "  let initialTimezone='local'; try { const savedTimezone=localStorage.getItem(TIMEZONE_STORAGE_KEY); if(savedTimezone==='local'||savedTimezone==='utc')initialTimezone=savedTimezone; } catch (_) {}",
