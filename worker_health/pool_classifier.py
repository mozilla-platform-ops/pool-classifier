...
class PoolClassifier:
    ...
    def _write_html(
        ...
        parts += [
            ...
            "  function renderLag(data) { const usable = data.buckets.filter(b => b.sufficient_samples); if (!usable.length) { lagChart.innerHTML = `<text x='24' y='35' fill='#888'>Not enough observed terminal runs yet (need ${data.min_samples} per hour).</text>`; return; } const first=data.buckets.indexOf(usable[0]), last=data.buckets.indexOf(usable.at(-1)), span=data.buckets.slice(first,last+1), maxLag=Math.max(data.slo_seconds,...usable.map(b=>b.p95_seconds)), maxCount=Math.max(...span.map(b=>b.sample_count),1), x=i=>span.length===1?484:34+i*900/(span.length-1), y=v=>185-v/maxLag*145, points=key=>usable.map(b=>`${x(span.indexOf(b)).toFixed(1)},${y(b[key]).toFixed(1)}`).join(' '); const bars=span.map((b,i)=>`<rect x='${x(i)-2}' y='${214-b.sample_count/maxCount*22}' width='4' height='${b.sample_count/maxCount*22}' fill='#555'><title>${b.start_at}\\n${b.sample_count} samples\\np50 ${fmtLag(b.p50_seconds)}, p95 ${fmtLag(b.p95_seconds)}</title></rect>`).join(''); const dots=usable.flatMap(b=>[['p50_seconds','#5dd'],['p95_seconds','#f90']].map(([key,color])=>`<circle cx='${x(span.indexOf(b))}' cy='${y(b[key])}' r='3.5' fill='${color}'><title>${b.start_at}\\n${key === 'p50_seconds' ? 'p50' : 'p95'} ${fmtLag(b[key])}\\n${b.sample_count} samples</title></circle>`)).join(''); const lines=usable.length>=4?`<polyline points='${points('p50_seconds')}' fill='none' stroke='#5dd' stroke-width='2'/><polyline points='${points('p95_seconds')}' fill='none' stroke='#f90' stroke-width='2'/>`:''; lagChart.innerHTML=`<line x1='34' y1='185' x2='934' y2='185' stroke='#555'/><line x1='34' y1='${y(data.slo_seconds)}' x2='934' y2='${y(data.slo_seconds)}' stroke='#f44' stroke-dasharray='5 4'/><text x='38' y='${y(data.slo_seconds)-4}' fill='#f77' font-size='11'>SLO ${fmtLag(data.slo_seconds)}</text><text x='34' y='235' fill='#777' font-size='11'>${span[0].start_at.slice(5,16)} UTC</text><text x='800' y='235' fill='#777' font-size='11'>${span.at(-1).start_at.slice(5,16)} UTC</text>${bars}${lines}${dots}`; }",
