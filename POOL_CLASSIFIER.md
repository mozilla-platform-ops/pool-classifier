...
- Admin dashboard: <http://localhost:8080/admin> (enabled by the local `pc_start.sh` helper)
...
| `ADMIN_IAP_BYPASS`        | Set to `1` only for local development to allow `/admin` without IAP | `1` from `pc_start.sh`; unset otherwise (deny) |
